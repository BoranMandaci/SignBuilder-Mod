...
*   🏢 **Automatic 2x2 Multi-Block Signs** — Need bigger signage? Placing four matching sign blocks in a 2x2 square seamlessly merges them into a giant 2.0x scaled sign! Features synchronized block breaking, multi-block backplates (consuming and refunding 4x materials), dynamic hitboxes, off-screen FOV protection, and master-dummy syncing across both wall and floor orientations.
...
*   🧱 **28 Dynamic Materials & GUI Pagination** — Signs are no longer just concrete! Customize your text with 28 distinct materials—including Redstone Block, Netherite Block, Quartz Block, Emerald Block, Purpur Block, Polished Granite, Stone, Smooth Sandstones, woods, and precious metals. Browse them effortlessly in the Paint Brush screen using dedicated page-turning (`<` / `>`) buttons.
...
*   🏗️ **Expanded 3D Characters & International Support** — Fully modeled character set including Latin letters (A-Z), German & Turkish special letters (`Ä`, `ß`, `Ç`, `Ğ`, `İ`, `Ö`, `Ş`, `Ü`), numbers (0-9), currency symbols (€, $, ₺, ¥), comparison operators (`<`, `>`), directional arrows, math operators, and punctuation (`'`, `:`, `;`, `!`, `?`, `=`, `÷`, `"`, `#`, `@`, `&`, `%`, `*`). All blocks feature pixel-accurate hitboxes for wall, floor, and ceiling placements.
...
*   🗺️ **Holographic Blueprint, Vertical Placement & Undo** — Type your text into the redesigned compact 2-row Blueprint GUI. Enjoy real-time **translucent 3D ghost previews** directly in the world, dynamically shifting between **green** (valid) and **red** (obstructed). Supports horizontal or Y-axis locked **vertical placement**, instant **2x2 multi-block sizing**, and an **automatic backplate toggle**. Made a mistake? The one-click Undo system removes the construction and refunds all blocks and backplates.
